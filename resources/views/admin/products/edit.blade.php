<x-app-layout>
        ...
        <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data"
            ...
            <div class="p-6 space-y-6">
                ...
                <section x-show="step === 3" x-cloak class="space-y-6">
                    ...
                    <input type="hidden" name="variants_payload"
                        :value="(mode === 'edit' && options.length === 0 && variants.length === 0) ? '' : JSON.stringify(variantsPayload())">
                                        ...
                                        class="rounded border-slate-300">
                                            ...
                                            <td class="px-4 py-4 align-top">
                                                <select
                                                    ...
                                                    @php($pm = old('finishing_config.'.$f->id.'.pricing_mode', data_get($productPayload, 'finishing_config.'.(string) $f->id.'.pricing_mode', 'per_piece')))
                                                    <option value="per_piece" @selected($pm === 'per_piece')>Per piece</option>
                                                    <option value="per_side" @selected($pm === 'per_side')>Per side</option>
                                                    <option value="flat" @selected($pm === 'flat')>Flat</option>
                                                </select>
                                            </td>
                                            ...
                                            <td class="px-4 py-4 align-top">
                                                <input type="number" min="0"
                                                    ...
                                                    value="{{ old('finishing_config.'.$f->id.'.min_qty', data_get($productPayload, 'finishing_config.'.(string) $f->id.'.min_qty')) }}"
                                                    placeholder="0"
                                            ...
                                            <td class="px-4 py-4 align-top">
                                                <input type="number" min="0"
                                                    ...
                                                    value="{{ old('finishing_config.'.$f->id.'.max_qty', data_get($productPayload, 'finishing_config.'.(string) $f->id.'.max_qty')) }}"
                                                    placeholder="—"
                                                    class="w-24 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm focus:border-slate-900 focus:ring-2 focus:ring-slate-900/10 disabled:bg-slate-100 disabled:text-slate-500">
                                            </td>
                                            ...
                                            <td class="px-4 py-4 align-top">
                                                <label class="inline-flex items-center gap-2">
                                                    <input type="checkbox"
                                                        ...
                                                        value="1"
                                                        @checked((bool) old('finishing_config.'.$f->id.'.is_required', data_get($productPayload, 'finishing_config.'.(string) $f->id.'.is_required', false)))
                ...
                <section x-show="step === 8" x-cloak class="space-y-6">
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                            <h3 class="text-lg font-bold text-slate-900">Review & Update</h3>
                            <p class="text-sm text-slate-600">
                                Quick preview of how this product will behave in the storefront once updated. You can go back and adjust any step.
                            </p>
                        </div>
                    ...
                    <div class="rounded-3xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
                        <div class="border-b border-slate-100 bg-slate-50/60 px-6 py-4 flex items-center justify-between">
                            ...
                            <div class="text-xs text-slate-500">Visual summary only · final data saves on Update</div>
                        </div>
    ...
    {{-- Alpine logic --}}
    <script>
        function productWizard({ initialStep = 1, old = {}, product = null, mode = 'create' }) {
            const base = product || {};
            ...
            return {
                ...
                steps: [
                    ...
                    { key: 'review', label: 'Review & Update', short: 'Review' },
                ],
                ...
                pricing: {
                    min_qty: old?.pricing?.min_qty ?? base?.pricing?.min_qty ?? 1,
                    allow_tiers: old?.pricing?.allow_tiers ?? '1',
                    rate_per_sqft: old?.pricing?.rate_per_sqft ?? base?.pricing?.rate_per_sqft ?? '',
                    offcut_per_sqft: old?.pricing?.offcut_per_sqft ?? base?.pricing?.offcut_per_sqft ?? '',
                ...
                seo: {
                    ...
                    seo_description: old?.seo?.seo_description ?? base?.seo?.seo_description ?? '',
                    seo_keywords: old?.seo?.seo_keywords ?? base?.seo?.seo_keywords ?? '',
                    og_title: old?.seo?.og_title ?? base?.seo?.og_title ?? '',
                    og_description: old?.seo?.og_description ?? base?.seo?.og_description ?? '',
                    canonical_url: old?.seo?.canonical_url ?? base?.seo?.canonical_url ?? '',
                ...
                finishingSearch: '',
                selectedFinishingIds: @json(collect(old('finishings', $productPayload['finishing_ids'] ?? []))->map(fn($v) => (string) $v)->values()),

                init() {
                    const hasOld = !!Object.keys(old || {}).length;

                    if (!hasOld && mode === 'create') {
                        try {
