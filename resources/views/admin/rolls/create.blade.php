<x-app-layout>
    ...
    <x-slot name="pageTitle">New Roll</x-slot>
    ...
    <div class="space-y-6">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="space-y-1">
                    <h2 class="text-xl font-bold leading-tight tracking-tight">
                        Define a new roll width
                    </h2>
                    <p class="text-xs sm:text-sm text-white/85">
                        Rolls are reusable physical widths (flex, sticker, vinyl, banners) that your dimension-based
                        products and pricing can share across the system.
                    </p>
                    <div class="mt-2 inline-flex items-center gap-2 rounded-full bg-black/10 px-3 py-1 text-xs">
                        <span class="h-1.5 w-1.5 rounded-full bg-emerald-300"></span>
                        <span>Create once, then attach to multiple products and pricing contexts.</span>
                    </div>
                </div>
            </div>
        ...
        <div class="rounded-3xl border border-slate-200/80 bg-white px-5 py-5 shadow-sm sm:px-6 sm:py-6">
            <form method="POST" action="{{ route('admin.rolls.store') }}" class="space-y-6">
                @csrf

                @include('admin.rolls._form')
